...
- [ ] Google로 로그인하면 계정이 자동 생성되고 `member`가 된다 — **실제 자격증명 필요**
- [ ] `ADMIN_EMAIL` 계정으로 로그인하면 `admin`이 된다 — **실제 자격증명 필요**
...
> **미검증 2건.** Google 코드 교환 이후 경로(계정 자동 생성, 마스터 승격)는
> 실제 OAuth 클라이언트가 있어야 확인된다. 그 앞뒤는 전부 검증했다 —
> 동의 화면 리다이렉트와 `state` 쿠키 발급, 코드 교환 뒤의 사용자 upsert·역할 판정
> (단위 테스트), 세션 발급·조회·만료·폐기(통합 테스트).
> **자격증명을 넣은 뒤 이 두 줄을 직접 확인할 것.**

## 7. 다음

- 편집·검수 CSR 화면 (이제 만들 수 있다)
- 배포 (Railway) — **이 슬라이스가 끝나야 시작할 수 있다**
- 역할 관리 UI, 도서 목록·검색, 관리자 확인 큐 화면
