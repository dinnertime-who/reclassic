...
> **갱신 ([ADR-040](ADR-040.md), 2026-09-03):** 이 가운데 **스크립트 금지가 풀렸다.**
> 읽기 설정을 위해 작은 스크립트 하나가 들어간다.
>
> **갱신 ([ADR-043](ADR-043.md), 2026-09-04):** **shadcn 금지도 위젯에 한해 풀렸다** —
> 다이얼로그·시트처럼 접근성 동작이 붙는 것은 손으로 만들지 않는다.
> **react-query 훅 금지만 남는다.**
