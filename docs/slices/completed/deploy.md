...
**작업 종류:** 구현 슬라이스 (인프라)
**선행 슬라이스:** `docs/slices/completed/auth.md` — 인증이 없으면 배포하면 안 된다.
임시 헤더를 걷어낸 뒤에야 시작할 수 있다.
**다음 슬라이스:** 편집·검수 화면 — 아직 명세 없음
**선행 문서:** `AGENTS.md`, `docs/ARCHITECTURE.md` (특히 "핵심 불변식 4"),
`docs/decisions/index.md` (특히 ADR-001 / 002 / 008 / 019 / 022 / 026 / 027 / 028 / 029 / 030 / 031 / 032 / 033)
...
> **1. 맞물린다.** `api`가 `[::]:8080`에 뜨고 SSR이 `api.railway.internal`로 부른다.
>    같은 HTML의 로그인 링크는 공개 주소다 — 서버와 브라우저가 실제로 갈린다.
> **2. 공유된다.** `COOKIE_DOMAIN=.dinnertimes.app`으로 두 서브도메인이 세션을 나눠 쓴다.
> **3. 아프지 않았다.** 손으로 끝까지 갔고 막힌 곳이 없었다.
>    **안 C(Cloudflare 이전)를 지금 열 근거는 없다.**

막힌 것은 전부 값이 안 들어간 것이었다 — `GOOGLE_CLIENT_*` 둘과 `R2_*` 셋.
구조 문제가 아니었고, 코드는 기동 시점에 정확히 어느 키가 비었는지 말해 줬다
(CONVENTIONS "기본값을 조용히 채우지 않는다"가 값을 한 자리다).
...
### 6.4 Railway 대시보드 — 확인 완료 (ADR-031)

**2026-08-25 실배포로 둘 다 결론이 났다.** 서비스당 한 번씩 대시보드에서 하면 된다.

| 항목 | 결과 |
|---|---|
| **서비스별 설정 파일 경로** (`railway.api.json` 등) | **확인됨.** 서비스 Settings에 **저장소 절대 경로**(`/railway.api.json`)를 넣으면 읽힌다. 서비스 레벨 builder가 `RAILPACK`으로 남아 있어도 **파일이 이겨서** Dockerfile로 빌드된다 |
| **메모리 상한** (§4.2) | **확인됨 — 먹는다.** 메트릭 `MEMORY_LIMIT_GB`가 파일 값 그대로다: `worker` 1.073741824GB · `api` 0.536870912GB. 공개 설정 목록에 없을 뿐 스키마대로 반영된다 |

결과는 ADR-031의 해당 항목에 이어 적었다.
...
**Railway 실물 검증 (2026-08-25).** 아래는 전부 프로덕션에서 확인한 것이다.

- [x] 4서비스가 Railway에 뜨고 `/healthz`가 `db: ok`를 준다
- [x] **서비스별 `railway.json`이 실제로 읽힌다** (ADR-031) —
      서비스 builder가 `RAILPACK`인데도 Dockerfile로 빌드됐다. 파일이 이긴다
- [x] **SSR이 `api.railway.internal`로 API를 부른다** — 공개 도메인을 경유하지 않는다 (불변식 4).
      `api 기동 addr="[::]:8080"` · web 배포 시각에 api가 `/healthz`·`/auth/me`를 받았다.
      같은 HTML의 로그인 링크는 빌드 시점에 박힌 공개 주소다 — **서버와 브라우저가 실제로 갈린다**
- [x] 프로덕션 도메인에서 Google 로그인이 되고, `ADMIN_EMAIL` 계정이 `admin`이 된다 —
      `로그인 handle="dinnertime-dev" role="admin"`
- [x] **웹과 API가 다른 서브도메인인데 세션 쿠키가 공유된다** (`COOKIE_DOMAIN`).
      콜백이 심은 쿠키를 브라우저가 웹 출처로 보냈고, SSR이 그것을 내부 호출로 전달해 `/auth/me` 200을 받았다 —
      직전까지 같은 경로가 전부 401이었다
- [x] 관리자가 도서 한 권을 지시하면 수집·파싱·적재가 끝까지 돈다 —
      **1342 오만과 편견, 지시부터 적재까지 2.5초.** 아래 "골든 대조" 참조
- [x] 읽기 화면이 프로덕션에서 뜨고 `noindex`가 붙어 있다 —
      `/books/1342/chapters/5` 200 · `<meta name="robots" content="noindex, follow"/>` (ADR-007)
- [x] **pre-deploy 명령으로 마이그레이션이 배포당 한 번만 실행된다** (ADR-030) —
      goose가 v3까지 올리고 River 마이그레이션까지 이어 돈 뒤 컨테이너가 내려가고, 그다음 본 기동이 붙는다
- [ ] 마이그레이션이 실패하면 새 버전이 뜨지 않는다 —
      **미검증. 일부러 깨야 확인되는 항목이라 프로덕션에서 하지 않았다.**
      별도 환경이 생길 때 확인한다
- [x] **§6.4 미확인 둘의 결과가 ADR-031에 적혔다**
- [x] `make lint && make test` 통과. `DATABASE_URL` 없이도 통과한다.
      다만 **`golangci-lint`가 없어 `go vet`으로 대체됐다** — 의도한 강도가 아니다.
      CI를 넣을 때 여기서 버전을 고정한다

**골든 대조 — `internal/parse/testdata/golden/1342.json`과 전부 일치**

| | 골든 | 프로덕션 |
|---|---|---|
| `source_sha256` | `6ac95942…3848a` | **동일** |
| `chapter_count` | 63 | **63** |
| `paragraph_count` | 2131 | **2131** |

`source_sha256`이 같다는 것이 핵심이다. 프로덕션이 Gutenberg에서 받은 원본이 로컬 코퍼스와
**바이트 단위로 같은 파일**이고, 그 위에서 파서가 로컬과 같은 수를 냈다.
입력도 파서도 프로덕션에서 갈리지 않았다. R2에는 852,578바이트가
`sources/1342/<sha256>.html`로 올라갔다 — 키 형식이 `SourceKey` 규약과 맞는다.

**메모리는 아직 실측이 아니다.** `worker` 사용량이 최대 20.6MB로 찍혔지만
샘플링이 60초 간격이고 파싱은 2초에 끝났다 — **피크를 놓친 값이다.**
§4.2가 예측한 1342의 29MB, 셰익스피어의 310MB는 여전히 미검증이다.

---

## 9. 다음

**ADR-028의 숙제는 닫혔다** — 운영이 아프지 않았으므로 안 C(Workers + Hyperdrive + Postgres)를
열지 않는다. **화면 슬라이스(7)로 간다.**

`.railway/railway.ts`(TypeScript IaC)로 옮길 이유도 아직 없다.
