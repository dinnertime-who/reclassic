...
**구현은 로컬 검증까지 끝났고, 프로덕션 확인이 남았다.** 슬라이스를 `completed/`로 옮기지 않는다.
D1~D5는 닫혔다. 목록 응답 `{ items }`는 [ADR-037](../decisions/ADR-037.md) **Proposed** —
사용자 확인을 기다린다.

**슬라이스 7이 넘긴 숙제 하나:** 프로덕션에서 **제안 → 승인이 실제로 도는 것**을 아직 못 봤다.
로컬에서는 돌았다(제안 1115번, reviewer `alice`가 승인). **프로덕션은 배포 뒤에 한 번 확인한다.**
...
### 8. 도서 목록 · 관리자 확인 큐 (로컬까지)

**스키마와 ADR이 만들어 둔 `needs_review`·고아·`reviewer`·`published`에 손이 닿는다.**
D1~D5가 닫혔다. 목록 응답은 `{ items }`([ADR-037](../decisions/ADR-037.md) Proposed).
...
**프로덕션 확인은 아직이다.** 슬라이스 7이 넘긴 숙제(프로덕션에서 제안 → 승인)도 로컬에서만
돌았다. 그래서 `completed/`로 옮기지 않는다.
