...
**신뢰도는 전략이 반환하지 않는다.** 전략은 추출 결과만 내고, 점수는 `internal/parse/confidence.go`가
결과를 보고 따로 계산한다. 전략마다 자기 점수를 매기면 서로 비교할 수 없기 때문이다.
모든 전략을 돌린 뒤 최고 점수를 채택한다.

신뢰도가 임계값 미만이면 자동 확정하지 않고 **관리자 수동 검토 큐**로 보낸다.
전부 자동으로 처리하려 들지 않는다.

채택된 결과에는 ADR-013의 후처리를 적용하고, 최종 신뢰도는 후처리 후에 다시 계산한다.
...
- **같은 본문이 한 책에 반복되면 2번째부터 등장 순서를 붙인다** (ADR-016)
  ```
  n = 1  →  sha256(normalized)[:16]
  n ≥ 2  →  sha256(normalized + "#" + n)[:16]
  ```
  붙이지 않으면 반복되는 대사가 번역 행 하나를 공유해, 화자가 달라도 같은 존댓말이 강제된다.
  순서 부여는 파서 후처리에서 한다. 정규화 함수는 순수 함수로 유지한다.
...
**위치를 해시에 넣지 말 것.** 챕터 하나만 지워져도 뒤 문단 전체의 id가 밀려
재파싱 승계가 전부 깨진다. 검토 근거는 ADR-016.
...
2번의 `AND status = 'pending'`은 **같은 제안**을 두 검수자가 동시에 승인하는 경우를 막는다.
영향 행이 0이면 롤백하고 409를 반환한다.

**서로 다른 제안을 같은 문단에 동시 승인하는 경우는 이것으로 막히지 않는다.**
둘 다 1번에서 "확정본 없음"을 보고 아무것도 supersede하지 않아, 확정본은 1행인데
`approved` 제안이 2개 남는다. 그래서 트랜잭션 맨 앞에서 문단 단위 자문 잠금을 잡는다:

```sql
SELECT pg_advisory_xact_lock(hashtextextended(project_id || ':' || stable_id, 0));
```

근거와 재현 결과는 ADR-024. **이 잠금을 빼지 말 것** — 단발 테스트로는 통과한다.
