...
**현재 작업은 슬라이스 5 — 세션 인증입니다.**
지금 신원은 `X-User-Handle` 헤더입니다. **인증이 아닙니다. 위조가 자명하게 가능합니다.**
걷어내기 전까지 프로덕션에 배포하지 마세요.
...
- **`X-User-Handle` 헤더를 인증으로 취급하지 마세요.** 임시 신원이고 위조가 가능합니다.
