...
- **읽기 화면에 react-query 훅이나 shadcn 컴포넌트를 넣지 마세요** (ADR-035·040).
  지켜야 할 것은 **본문이 JS 없이 HTML에 담겨 오는 것**입니다(ADR-007·023 SEO 전제) — 읽기 설정 스크립트는 그 위에 얹습니다. **깨져도 SSR은 멀쩡해서 배포는 성공으로 보입니다.**
